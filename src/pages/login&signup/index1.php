<?php
session_start();
include_once __DIR__."\..\admin\database\conn.php";
include_once __DIR__."\..\admin\Requests\Validation.php";
...
if($_SERVER["REQUEST_METHOD"] === "POST" ){
    ...
    //  $imageName=$_FILES['company_logo']['name'];
...
if($firstnameResult){
    ...
    // $firstnamePatternResult=$firstnameValidation->regex($firstnamePattern);
...
if(isset($job_other )){
  ...
  if(isset($job_other ) && $jobOtherValidation->hasErrors())
  {
...
if(empty($fnameErrors) && empty($lnameErrors) && empty($emailErrors) && empty($passwordErrors) && 

 empty($whatsappNumberErrors) && empty($jobErrors) && empty($jobOtherErrors) && empty($imageValidationErrors) )
 {
...
?>
<!-- <input class="btn btn-primary text-white" name="regForm"  type="submit" value="Get free access"/> 
